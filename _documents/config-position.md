...
* 定位规则和**定位描述对象**一致。
* key 设置为空串或空白字符也会认为是有效的 key，如空花括号 `{}` 表示 key 是一个空串；花括号中间两个空格 `{  }` 表示 key 是两个空格。
* priority 设置为空串或空白字符认为未设置 priority，如 `()` 或 `(  )` 都表示不指定 priority，若非空则必须是有效数字。
* key 和 priority 的位置可以互换，可以分别在字符串首或串尾，但不能出现在 path、name、extension 之间。
* 目前仅对括号顺序错乱的情况进行报错提醒，如 `}xxx{`、`(xxx{xxx)` 等。
